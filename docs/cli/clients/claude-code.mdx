...
- **Write access** to `~/.vscode` (VS Code extensions directory)
- **Write access** to the OS-specific VS Code app data directory:
  `~/Library/Application Support/Code` on macOS, `~/.config/Code` on Linux
- **Read access** to `~/.local/share/claude` (Claude Code installation path)
- **Read access** to `~/Library/Keychains/login.keychain-db` on macOS
