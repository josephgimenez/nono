...
**Groups:** `claude_code_macos`, `user_caches_macos`, `node_runtime`, `rust_runtime`, `python_runtime`, `vscode_macos`, `vscode_linux`, `unlink_protection` (plus all base_groups)

**Filesystem:** `~/.claude` (read+write), `~/.claude.json` (read+write), `~/.local/share/claude` (read), plus the macOS keychain path and read+write OS-specific VS Code paths from the matching platform groups
