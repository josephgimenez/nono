...
**Filesystem:** `~/.claude` (read+write), `~/.claude.json` (read+write), `~/.local/share/claude` (read), plus the macOS keychain path and OS-specific VS Code paths from the matching platform groups
